...
public interface TransactionRepository extends CrudRepository<Transaction, String> {
    @Query(value = "SELECT sum(amount) FROM transactions WHERE account_number = :accountNumber", nativeQuery = true)
    BigDecimal findNetworthOfAccount(@Param("accountNumber") Long accountNUmber);
...
//    @Query(value = "SELECT sum(amount) FROM transactions t JOIN accounts a ON t.account_id = a.id WHERE a.account_type not in ('CREDITLINE')", nativeQuery = true)
    ...
    List<Transaction> findByAccountNumber(Long accountId);
}
